...
TMGClothing.ResetItemTexture = function(obj, category) {
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            ...
            if (clothingCategory == "model") {
                ...
                if (buttonType == "item") {
                    TMGClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
TMGClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function() {
    window.addEventListener('message', function(event) {
        ...
        switch (event.data.action) {
            case "open":
                TMGClothing.Open(event.data);
                break;
            case "close":
                TMGClothing.Close();
                break;
            case "updateMax":
                TMGClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                TMGClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                TMGClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                TMGClothing.ResetValues();
                break;
...
TMGClothing.ReloadOutfits = function(outfits) {
...
TMGClothing.Open = function(data) {
    ...
    TMGClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    TMGClothing.SetCurrentValues(data.currentClothing);
...
TMGClothing.SetMaxValues = function(maxValues) {
...
TMGClothing.ResetValues = function() {
