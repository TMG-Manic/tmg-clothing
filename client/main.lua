...
RegisterNetEvent('tmg-clothing:client:loadOutfit', function(oData)
    ...
    if data["accessory"] ~= nil then
        if TMGCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if Config.UseTarget then
        CreateThread(function()
            for k, v in pairs(Config.Stores) do
                ...
                exports['tmg-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                    name = v.shopType .. k,
                    debugPoly = false,
                    minZ = v.coords.z-1,
                    maxZ = v.coords.z+1,
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not TMGCore.Shared.TMGJobsStatus then
                    action = function()
                        customCamLocation = v.cameraLocation
                        local gradeLevel = PlayerData.job.grade.level
                        getOutfits(gradeLevel, Config.Outfits[v.requiredJob])
                    end
                else break end --this break is important if TMGJobsStatus is true then the reste of the code in this loop does not need to run.

                exports['tmg-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
	    ...
	    for k, v in pairs(Config.OutfitChangers) do
                exports['tmg-target']:AddBoxZone('OutfitChangers_'..k, v.coords, v.length, v.width, {
                    ...
                    options = {
                        {
                            type = "client",
                            event = "tmg-clothing:client:openOutfitMenu",
                            icon = "fas fa-sign-in-alt",
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'surgeon' then
                        exports['tmg-core']:DrawText('[E] - '..Lang:t("store.surgeon"), 'left')
                    elseif zoneName == 'clothing' then
                        exports['tmg-core']:DrawText('[E] - '..Lang:t("store.clothing"), 'left')
                    elseif zoneName == 'barber' then
                        exports['tmg-core']:DrawText('[E] - '..Lang:t("store.barber"), 'left')
                    elseif zoneName == 'outfit' then
                        exports['tmg-core']:DrawText('[E] - '..Lang:t("store.outfitchanger"), 'left')
                    end
                else
                    inZone = false
                    ...
                    if PlayerData.gang and PlayerData.gang.name or (not TMGCore.Shared.TMGJobsStatus and PlayerData.job.name) then
                        ...
                        clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                            if isPointInside then
                                local zoneID = tonumber(TMGCore.Shared.SplitStr(zone.name, "_")[2])
                                local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or (not TMGCore.Shared.TMGJobsStatus and PlayerData.job.name)
                ...
                if inZone then
                    ...
                    elseif not TMGCore.Shared.TMGJobsStatus then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or (not TMGCore.Shared.TMGJobsStatus and PlayerData.job.grade.level)
